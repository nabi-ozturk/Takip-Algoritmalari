...
if len(track_df) != 0:
    ...
    plt.plot(np.sqrt(gt_center_x-track_df_center_x)**2 + (gt_center_x-track_df_center_y)**2)
    plt.xlabel("frame")
    plt.ylabel("Öklid mesafesi btw gt ve track")
    error = np.sum(np.sqrt(gt_center_x-track_df_center_x)**2 + (gt_center_x-track_df_center_y)**2)
